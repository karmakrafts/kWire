...
actual inline operator fun NIntArray.plus(other: NIntArray): NIntArray = NIntArray(size + other.size).apply {
    usePinned { destArray ->
        this@plus.usePinned { sourceArray ->
            memcpy(destArray.addressOf(0), sourceArray.addressOf(0), (this@plus.size * sizeOf<LongVar>()).convert())
        }
        other.usePinned { sourceArray ->
            memcpy(destArray.addressOf(this@plus.size), sourceArray.addressOf(0), (other.size * sizeOf<LongVar>()).convert())
        }
    }
}
