...
internal class IntrinsicsValidationVisitor(
    context: KWirePluginContext
) : IrVisitorVoid(), MessageCollectorExtensions by context {
    override fun visitCall(expression: IrCall) {
        super.visitCall(expression)
        val function = expression.target
        if (function.getIntrinsicType() == null) return
        reportError(
            "Encountered an intrinsic that wasn't lowered. This is a kWire compiler bug, please report it at https://github.com/karmakrafts/kWire/issues",
            expression
        )
    }
}